export const fetchCategories = async (baseUrl: string, locale: LocaleT) => {
    ...
    const categoryNewsResponse = await fetch(`${baseUrl}/wp-json/wp/v2/categories?slug=${newsSlugObject[locale]}&hide_empty=true`);
    ...
    const childCategoriesResponse = await fetch(`${baseUrl}/wp-json/wp/v2/categories?parent=${categoryId}&orderby=count&order=desc`);
